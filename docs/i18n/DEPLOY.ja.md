...
Fulgoria には、静的アプリの配信と `.env` からの任意の**ログイン**処理だけを行う**軽量サーバー**
...
ログインを使う場合（公開する用途では推奨）、`.env` に次を設定します：
...
AUTH_ENABLED=true
AUTH_USER=diego
AUTH_PASSWORD=あなたのパスワード   # 平文、Escriba/Fisherboy と同じ
SESSION_SECRET=<下で生成した値を貼る>
COOKIE_SECURE=true               # HTTPS の背後では true、ローカル http:// のときだけ false
```

生成が必要なのは `SESSION_SECRET` だけです（セッション cookie に署名します）：

```bash
openssl rand -hex 32
# Windows に openssl が無い場合 -> node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"
```

> **パスワードを平文で置きたくない？** 代わりに bcrypt ハッシュを使います：`node server.js --hash 'あなたのパスワード'` を実行し、`$2a$...` を `AUTH_PASSWORD` に貼ります（サーバーが自動判別）。
>
...
```bash
cp .env.example .env          # AUTH_PASSWORD+ SESSION_SECRET を記入（または AUTH_ENABLED=false）
...
```bash
docker logs -f fulgoria     # ログを追う
docker rm -f fulgoria       # 停止して削除
```
...
2. イメージ：`ghcr.io/diegoparras/fulgoria:latest`
...
1. **Create → App → GitHub repo** で `diegoparras/fulgoria` を指定。
...
Fulgoria をあなた以外に公開する前に：

- [ ] `AUTH_USER`、`AUTH_PASSWORD`（パスワード、または bcrypt ハッシュ）、`SESSION_SECRET` を設定。
